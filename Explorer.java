...
public class Explorer {
    ...
    private boolean dfs(ExplorationState state, Set<Long> visited) {
        ...
        // Base case: found the orb
        if (state.getDistanceToTarget() == 0) {
            return true;
        }

        // Sort neighbors by increasing distance to the Orb (greedy DFS)
        List<NodeStatus> neighbors = new ArrayList<>(state.getNeighbours());
        neighbors.sort(Comparator.comparingInt(NodeStatus::getDistanceToTarget));

        for (NodeStatus neighbor : neighbors) {
            if (!visited.contains(neighbor.getId())) {
                state.moveTo(neighbor.getId());
                if (dfs(state, visited)) {
                    return true;  // Found the orb in deeper path
                }
                // Backtrack
        ...
        return false;  // Orb not found in this path
    }
